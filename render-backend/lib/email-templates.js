...
// =================== RESUMEN DIARIO (1 correo al día, no uno por venta) ===================
function buildDailySummary(stats) {
    const st = stats || {};
    const items = Array.isArray(st.items) ? st.items : [];
    const lineas = items.map((s) => {
        const nombre = esc(s.productName || s.title || 'Producto');
        const cant = esc(s.quantity || 1);
        const total = clp(s.totalPrice != null ? s.totalPrice : (s.salePrice || 0) * (s.quantity || 1));
        return `<tr>
          <td style="padding:8px 0;border-bottom:1px solid ${BRAND.border};color:${BRAND.text};font-size:14px;">• ${nombre} <span style="color:${BRAND.muted};">× ${cant}</span></td>
          <td style="padding:8px 0;border-bottom:1px solid ${BRAND.border};color:${BRAND.text};font-size:14px;text-align:right;font-weight:600;">${total}</td>
        </tr>`;
    }).join('');
    const inner = `
      <p style="margin:0 0 16px;font-size:15px;line-height:1.5;color:${BRAND.text};">
        Hola, aquí está tu <strong>resumen de ventas de hoy</strong>:
      </p>
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin-bottom:18px;">
        ${lineas || `<tr><td style="color:${BRAND.muted};font-size:14px;padding:8px 0;">Sin ventas registradas.</td></tr>`}
      </table>
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
        ${row('Total del día', clp(st.revenue || 0), { strong: true })}
        ${row('Ganancias del día', clp(st.profit || 0), { strong: true, color: (st.profit || 0) >= 0 ? BRAND.good : BRAND.bad })}
      </table>`;
    return {
        subject: `📦 Tus ventas de hoy — ${st.periodLabel || ''}`.trim(),
        html: layout('Tus ventas de hoy', BRAND.primary, inner, st.periodLabel || 'Hoy')
    };
}
